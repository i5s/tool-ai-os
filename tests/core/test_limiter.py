...
@pytest.fixture
def limiter(cm: ConnectionManager) -> Limiter:
    return Limiter(cm=cm)


def test_can_use_within_limit(limiter: Limiter, storage):
    storage.set_config("daily_limit_opencode", "5")
    assert limiter.can_use("opencode") is True


def test_can_use_exceeds_limit(limiter: Limiter, storage):
...
def test_remaining_decreases_with_usage(limiter: Limiter, storage):
    storage.set_config("daily_limit_opencode", "5")
