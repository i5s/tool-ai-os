"""Proof: Sprint X-10 — Open Design in Runtime."""
...
def main() -> int:
    tmp = Path(tempfile.mkdtemp()) / "proof_opendesign.db"
    ...
    print("opendesign_id:", opendesign.id)

    runtime_svc = RuntimeService(cm)
    job = runtime_svc.create_runtime_job(
        task_id="proof-opendesign-sprint-x10",
        plan_text="Verify Open Design in Runtime alongside Hermes, OpenCode, and Ollama",
    )
    fragments = [
        {"fragment": "Acknowledge task", "agent_id": hermes.id},
        {"fragment": "Hi", "agent_id": opencode.id},
        {"fragment": "Say hi", "agent_id": ollama.id},
        {"fragment": "Create a simple logo concept", "agent_id": opendesign.id},
    ]
    ...
    assert len(conn.execute("SELECT * FROM runtime_assignments").fetchall()) == 4
    assert len(conn.execute("SELECT * FROM runtime_results").fetchall()) == 4
    assert len(conn.execute("SELECT * FROM agent_executions").fetchall()) == 4
